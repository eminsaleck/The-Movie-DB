        ...
        .sink(receiveValue: { [weak self] result in
          ...
          case .success(let newState):
            strongSelf.isFavorite.send(newState)
              strongSelf.closures?.updateFavoritesMovies?(MovieUpdated(movieId: strongSelf.movieId, isActive: newState))
          }
        })
        .store(in: &disposeBag)
    }
        ...
        .sink(receiveValue: { [weak self] result in
          ...
          case .success(let newState):
            strongSelf.isWatchList.send(newState)
              strongSelf.closures?.updateWatchListMovies?(MovieUpdated(movieId: strongSelf.movieId, isActive: newState))
          }
        })
        .store(in: &disposeBag)
    }
